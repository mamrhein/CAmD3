...
class Specification(Component):
    ...
    def __repr__(self) -> str:
        """repr(self)"""
        return "<%s x: %s>" % (self.interface.__name__, self._term('x'))
...
class NegatedSpecification(Specification):
    ...
    def _term(self, candidate_name: str) -> str:
        return "not (%s)" % self._spec._term(candidate_name)
...
class ValueSpecification(BaseSpecification):
    ...
    def is_satisfied_by(self, candidate: Any) -> bool:
        ...
        if isinstance(candidate, self._interface):
            try:
                attrVal = self._attr(candidate)
            except AttributeError:
                return False
            return self._operator(attrVal, self._value)
        return False
    ...
    def _negate(self) -> Specification:
        ...
        try:
            ...
            return NegatedSpecification(self)
        return self.__class__(self._interface, self._attr_name, invOp,
                              self._value)
